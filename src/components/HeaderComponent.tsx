...
const HeaderComponent = () => {
  ...
  return (
    <header className="fixed top-0 z-50 bg-gray-200 dark:bg-gray-800 w-full sm:p-1 md:p-2 opacity-90">
      <div className="layout flex justify-between items-center h-14 max-w-4xl mx-auto px-2 md:px-1">
        ...
        >
          {theme === "light" ? (
            <BiSun size={20} onClick={() => handleChangeTheme()} />
          ) : (
            <BiMoon size={20} onClick={() => handleChangeTheme()} />
          )}
        </button>
      </div>
    </header>
